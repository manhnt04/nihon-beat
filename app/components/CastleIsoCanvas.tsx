...
export default function CastleIsoCanvas({
  ...
  // Pan offset for camera movement
  ...
  // Guardian statue if equipped
  const guardianBuilding: IsoBuildingData | null = castle.decorations?.guardian
    ? {
        id: 'guardian-statue',
        name:
          castle.decorations.guardian === 'guardian-dragon'
            ? 'Thanh Long Trấn Thành'
            : castle.decorations.guardian === 'guardian-qilin'
            ? 'Kỳ Lân Hiến Thụy'
            : 'Thạch Sư Uy Nghi',
        hanzi:
          castle.decorations.guardian === 'guardian-dragon'
  ...
  const coreBuildings: IsoBuildingData[] = [
    {
      id: 'main',
  ...
  const allBuildings: IsoBuildingData[] = [
    ...coreBuildings,
  ...
  const render = useCallback(() => {
    ...
    const scaleFactor = Math.min(1.25, Math.max(0.72, width / 720));
    const tileW = BASE_TILE_W * scaleFactor;
    const tileH = BASE_TILE_H * scaleFactor;

    // Center the island with pan offset
    const originX = width / 2 + pan.x;
    const originY = height * 0.3 + pan.y;

    // --- 1. Draw Floating Island Cliff Depth (Underbelly) ---
    const islandN = gridToScreen(0, 0, originX, originY, tileW, tileH);
    ...
    const cliffDepth = 70 * scaleFactor;

    // Floating island underbelly shadow
    ctx.save();
    ctx.beginPath();
    ctx.ellipse(
      islandS_edge.x,
      islandS_edge.y + cliffDepth + 30,
      (GRID * tileW) / 2.2,
      tileH * 2.2,
      0,
      0,
    ...
    ctx.fillStyle = 'rgba(10, 5, 5, 0.35)';
    ...
    // --- 2. Draw Ground Tiles ---
    const isFrost = castle.theme === 'frost';
    const isLantern = castle.theme === 'lantern';
    const isCrimson = castle.theme === 'crimson';
    const isJade = castle.theme === 'jade';
    ...
    const sorted = [...allBuildings].sort((a, b) => depthKey(a) - depthKey(b));
    ...
    sorted.forEach((b, idx) => {
      ...
      ctx.fillStyle = 'rgba(15, 8, 7, 0.45)';
      ctx.filter = 'blur(6px)';
      ctx.fill();
      ctx.restore();
      ...
      // Center of roof for icon and Hanzi
      const roofCenterX = (Np.x + Ep.x + Sp.x + Wp.x) / 4;
      const roofCenterY = (Np.y + Ep.y + Sp.y + Wp.y) / 4;

      // Draw traditional pagoda tier lines for taller buildings
      if (b.h >= 2 && b.w >= 2) {
      ...
      // Footprint center for exact Labeling (Algorithm 2 note: centered on ground footprint)
      const footCenterX = (c.N.x + c.E.x + c.S.x + c.W.x) / 4;
      const footCenterY = (c.N.y + c.E.y + c.S.y + c.W.y) / 4;
  ...
  // Handle Resize and Animation Frame
  useEffect(() => {
    let animId: number;
    const handleResize = () => {
      const canvas = canvasRef.current;
  ...
  // Mouse & Touch Pointer Handling (Click & Pan)
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    const scaleFactor = Math.min(1.25, Math.max(0.72, rect.width / 720));
    const tileW = BASE_TILE_W * scaleFactor;
    const tileH = BASE_TILE_H * scaleFactor;
    const originX = rect.width / 2 + pan.x;
    const originY = rect.height * 0.3 + pan.y;
    ...
    if (hit) {
      if (placeMode) {
        onToast(`Ô đã có công trình [${hit.name}]`, 'bad');
      } else {
        onSelectBuilding(hit.id);
        onToast(`Đã chọn [${hit.name}] · ${hit.w}×${hit.h} ô`, 'ok');
      }
      return;
    }
    ...
    if (g.col >= 0 && g.row >= 0 && g.col < GRID && g.row < GRID) {
      if (placeMode) {
        if (canPlace(g.col, g.row, 1, 1)) {
          ...
          onToast(`Đã dựng Trạm gác mới tại (${g.col}, ${g.row})`, 'ok');
          if (onPlacedBuilding) onPlacedBuilding('Trạm gác mới');
          onSelectBuilding(newId);
        } else {
          onToast(`Ô (${g.col}, ${g.row}) đã bị chiếm dụng!`, 'bad');
        }
      } else {
        onSelectBuilding(null);
      }
    } else {
      if (!placeMode) onSelectBuilding(null);
    }
  };
